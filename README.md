...
1. Apply [`20260716120000_optional_email_reminders.sql`](supabase/migrations/20260716120000_optional_email_reminders.sql) after the existing reminder-outbox migrations.
2. Create a server-only Resend API key. Test with `FutureStack <onboarding@resend.dev>` and your Resend account email, then verify one custom domain before sending to other users.
...
4. Keep the GitHub Actions reminder dispatcher configured. It wakes the existing outbox; it does not send email directly.

The implementation is intentionally best-effort: an email provider error retries through the existing job lease, and a per-job record plus Resend idempotency protects duplicate sends. See [`docs/DOCUMENTATION.md`](docs/DOCUMENTATION.md) and [ADR-007](docs/adr/ADR-007-optional-email-reminders.md) for the limits and scale path.
