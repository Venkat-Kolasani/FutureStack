...
│   ├── src/content.js            # Optional on-demand page metadata listener
...
| Chrome extension | The MV3 side panel extracts LinkedIn, Greenhouse, Lever, JSON-LD, and Open Graph fields, stays open for multi-section copy/paste, then saves through the authenticated API. Production host permissions pin the Render API origin; localhost hosts are development-only. It is separately built and loaded, and needs the documented Clerk origin and CORS configuration. |
