...
The email channel follows [ADR-007](adr/ADR-007-optional-email-reminders.md) and [ADR-008](adr/ADR-008-user-controlled-email-reminders.md). It runs **after** the idempotent website notification is written and only when two independent conditions are true: the deployment has `REMINDER_EMAILS_ENABLED=true`, `RESEND_API_KEY`, and `REMINDER_EMAIL_FROM`; and that user has selected **Email deadline reminders**. The preference is default-off to avoid assuming consent, but it is the user's choice—not an administrator-controlled product choice.

Recipient resolution happens on the send path, not only at login. Clerk session JWTs do not include an email claim unless custom session tokens are configured, so `users.email` is often null after first sign-in. The dispatcher loads `email` and `clerk_id`, uses a stored address when it is valid, and otherwise calls Clerk's Backend API for the primary email, backfills `users.email`, then sends. A Clerk API/network failure is retryable through the existing job lease. If Clerk also has no usable email, the worker logs `REMINDER_EMAIL_SKIPPED_NO_RECIPIENT` (job id, user id, clerk id — never the address), skips email, and still completes the in-app notification. A configured Resend failure is retryable through the existing job lease and backoff, then appears in the existing dead-letter view.

`GET /api/v1/health/deps` reports `checks.reminderEmail.enabled` without leaking secrets. `true` means the three backend env vars are present; it does not mean a given user opted in. Render logs can be grepped for `REMINDER_EMAIL_SKIPPED_NO_RECIPIENT`, `REMINDER_EMAIL_BACKFILL_ERROR`, and `CLERK_EMAIL_LOOKUP_ERROR`. Already-completed jobs that skipped for a missing recipient are not resent; later reminders for that user will send once an address exists.
...
| Hackathon submission reminders | Transactional outbox, leased batches, in-app notifications, optional Resend email with Clerk recipient resolution and a per-job sent record, GitHub Actions best-effort scheduler. | Monitor queue age/dead jobs, Resend failures, skipped-no-recipient logs, and daily quota; add a dedicated scheduler/worker for time-sensitive delivery. | Independently scalable workers, provider webhooks, per-tenant channel preferences, and delivery-event audit streams. |
...
The current CRUD paths are synchronous because a user expects immediate feedback. The active-events migration narrows the existing job pattern—state, unique idempotency key, attempt count, lease, error reason, retry, and dead-letter state—to hackathon submission reminders. Their durable baseline is an in-app notification. When explicitly enabled, the worker adds an optional Resend email after that notification, resolving the recipient from `users.email` or Clerk, and records the provider message ID per job; the GitHub workflow only wakes dispatch and is not the durable queue itself.
