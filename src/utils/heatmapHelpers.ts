...
import type { HeatmapDay, HeatmapEntry, HeatmapGrid } from '../types';

type DateOptions = { today?: Date | string | null; dayCount?: number };

const resolveToday = (today: DateOptions['today']): Date =>
  today instanceof Date ? today : (parseLocalDate(today) ?? new Date());
...
export const getMondayStart = (date: Date): Date => {
 ...
 */
export const getIntensityLevel = (count: unknown): number => {
...
type IndexedDay = { date: string; count: number; tracks: string[] };

export const indexHeatmapData = (data: HeatmapEntry[] = []): Map<string, IndexedDay> => {
 ...
 */
export const buildHeatmapGrid = (data: HeatmapEntry[] = [], options: DateOptions = {}): HeatmapGrid => {
  const today = startOfLocalDay(resolveToday(options.today));
...
export const calculateCurrentStreak = (data: HeatmapEntry[] = [], options: DateOptions = {}): number => {
  const today = startOfLocalDay(resolveToday(options.today));
  const byDate = indexHeatmapData(data);
...
export const countLoggedDays = (data: HeatmapEntry[] = []): number => (
  [...indexHeatmapData(data).values()].filter((day) => day.count > 0).length
);

export const getBestWeekLoggedDays = (data: HeatmapEntry[] = [], options: DateOptions = {}): number => {
...
export const formatHeatmapTooltip = (day?: HeatmapDay | null): string => {
...
export const formatHeatmapRange = (rangeStart?: string | null, rangeEnd?: string | null): string => {
...
export const getCellAriaLabel = (day?: HeatmapDay | null): string => {
