export type TrackTemplateField = {
  key: string;
  label: string;
...
export function getTrackTemplate(type?: string | null): TrackTemplate {
  return TRACK_TEMPLATES.find((template) => template.type === type) || TRACK_TEMPLATES[TRACK_TEMPLATES.length - 1];
}

export function emptyMetadata(type?: string | null): Record<string, string> {
