...
/**
 * Generate a PDF report from opportunities data.
...
export const generatePDF = (
  ...
  const rejectionByOpportunityId = new Map<string, PipelineRejection>(
  ...
  const writeWrappedText = (text: string, indent = 0, fontSize = 10): void => {
    doc.setFontSize(fontSize);
    doc.setFont('helvetica', 'normal');
    const lines: string[] = doc.splitTextToSize(text, pageWidth - 2 * margin - indent);
    lines.forEach((line) => {
      checkPageBreak(6);
  ...
  if (pipelineAnalytics && pipelineAnalytics.rejectedCount > 0) {
    checkPageBreak(40);
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    ...
    if (pipelineAnalytics.rejectionByRoundType?.length) {
      checkPageBreak(20);
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
    ...
    if (pipelineAnalytics.rejections?.length) {
      checkPageBreak(20);
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
  ...
  opportunities.forEach((opp, index) => {
    ...
    const details = [
      `Category: ${titleCase(opp.category || 'internship')}`,
      `Status: ${titleCase(opp.status || 'applied')}`,
    ];
    ...
    details.forEach((detail) => {
      ...
      const lines: string[] = doc.splitTextToSize(detail, pageWidth - 2 * margin - 5);
      lines.forEach((line) => {
        checkPageBreak(6);
        doc.text(line, margin + 5, yPosition);
        yPosition += 5;
