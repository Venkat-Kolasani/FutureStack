...
const OPPORTUNITY_SHARE_FIELDS =
    'id, title, description, link, deadline, applied_on, status, category, campus_mode, created_at, rejected_round_number, current_round_number';
    ...
    try {
        ...
        if (internshipIds.length > 0 && fields?.rounds !== false) {
            const { data: rounds, error: roundsError } = await supabase
                ...
                .select('opportunity_id, round_number, round_type, scheduled_date, scheduled_time, result')
                .eq('user_id', userId)
